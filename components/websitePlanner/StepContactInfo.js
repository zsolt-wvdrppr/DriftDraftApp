  ...
  return (
    <form>
      <div className="md:py-10 max-w-screen-md mx-auto">
        <div className="p-2">
          <p className="mx-auto bg-yellow-100 whitespace-pre-line border-l-4 border-yellow-400 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100 p-4 rounded-xl text-sm max-w-xl self-center">{`These details will only be used if you request a quote. To do that, first complete the next step. Then, you’ll be able to request a quote under "My Activities".`}</p>
        </div>
            ...
            <div key={index} className="relative">
              ...
              {field.type === "text" && (
                <Input
                  ...
                  classNames={{
                    label:
                      "!text-primary dark:!text-accentMint text-sm md:text-md",
                  ...
                  startContent={
                    field.question.toLowerCase().includes("email") ?
                      <IconMail
                        className="text-primary dark:text-accentMint"
                        size={20}
                      />
                    : field.question.toLowerCase().includes("phone") ?
                      <IconPhone
                    ...
                    : field.question.toLowerCase().includes("website") ?
                      <IconWorldWww
                        ...
                        size={20}
                      />
                    : field.question.toLowerCase().includes("name") ?
                      <IconId
                        className="text-primary dark:text-accentMint"
                        size={20}
                      />
                    : field.question.toLowerCase().includes("employee") ?
                      <IconUsers
                        className="text-primary dark:text-accentMint"
                        size={20}
                      />
                  ...
                  value={
                    formValues[field.question.replace(/\s+/g, "").toLowerCase()]
                  }
                  onChange={(e) =>
                    handleChange(
                      field.question.replace(/\s+/g, "").toLowerCase(),
              ...
              {field.type === "select" &&
                field.question === "Are you a Start-Up?" && (
                  <RadioGroup
                    classNames={{
                      label:
                        "!text-primary dark:!text-accentMint text-sm md:text-md",
