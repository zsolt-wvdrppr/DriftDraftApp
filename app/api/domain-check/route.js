...
export async function GET(req) {
  ...
  try {
    ...
    const isDirectUkDomain = domain.endsWith('.uk') && 
    ...
    if (!availabilityResponse.ok) {
      ...
      if (isDirectUkDomain) {
        logger.debug(`GoDaddy API doesn't support direct .uk domains. Using fallback suggestions.`);
        
        // Return the fallback suggestions with a disclaimer
        return NextResponse.json({ 
          isAvailable: null, // We don't know for certain
          suggestions: fallbackSuggestions,
          message: "Direct .uk domains cannot be checked directly. Consider these alternatives:"
        });
      }
    ...
    if (isDirectUkDomain) {
      return NextResponse.json({ 
        ...
        suggestions: fallbackSuggestions,
        message: "Direct .uk domains cannot be checked via DriftDraft. Consider these alternatives:"
      });
    }
