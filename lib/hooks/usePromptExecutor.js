...
const usePromptExecutor = ({
  ...
  useEffect(() => {
    if (!jwt) {
      logger.warn("JWT is not available yet. Please log in.");

      return;
    }

    logger.info("JWT is available. Ready for prompt execution.");
  }, [jwt]);
  ...
  const executePrompts = async (prompts, userId, maxDepth = 3) => {
    ...
    // Calcualte the number of prompts to execute and add +3 for each prompt that has generateNewPrompts set to true (maxDeptht = 3 by default)
    const requiredCredits = prompts.length + prompts.filter((p) => p.generateNewPrompts).length * 3;

    const clientData = getClientData(); // Used strictly for rate limiting
    const results = []; // To store prompt outputs for dependencies

    if (!jwt) {
        logger.warn("No JWT token available. Please log in.");
        
        return [];
    }

    logger.debug("jwt available in usePromptExecutor:", jwt);
    ...
    for (let index = 0; index < prompts.length; index++) {
      ...
      try {
        ...
        if (!token)
          throw new Error(
            "Failed to generate a valid reCAPTCHA token. Please try again."
          );
        ...
        if (generateNewPrompts && maxDepth > 0) {
          ...
          logger.info(`Executing self-generated prompt at depth ${maxDepth - 1}`);
          
          await executePrompts(
            [{ prompt: expansionPrompt, generateNewPrompts: false }],
            userId,
            maxDepth - 1
          );
        }
  ...
  return { executePrompts, executedPrompts, output, loading, error, userMsg, hasCredits };
};
