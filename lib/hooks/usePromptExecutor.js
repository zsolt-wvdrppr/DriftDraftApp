...
const usePromptExecutor = ({ pickedModel, jwt, sessionData }) => {
  ...
  const executePrompts = async (prompts, userId, maxDepth = 3) => {
    ...
    for (let index = 0; index < prompts.length; index++) {
      ...
      try {
        ...
        // Step 1: Trigger background AI request
        const triggerResponse = await fetch("/api/trigger-ai-request", {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            prompt: finalPrompt,
            clientData,
            pickedModel: pickedModel || "gemini-2.5-flash",
          }),
        });

        if (!triggerResponse.ok) {
          const errorData = await triggerResponse.json();
          throw new Error(errorData.error || "Failed to start AI request");
        }

        const { requestId } = await triggerResponse.json();

        // Step 2: Poll for results
        const pollForResult = async (requestId) => {
          let attempts = 0;
          const maxAttempts = 300; // 15 minutes worth of polling (3-second intervals)

          while (attempts < maxAttempts) {
            const statusResponse = await fetch(
              `/api/check-ai-status/${requestId}`,
              {
                headers: {
                  Authorization: jwt ? `Bearer ${jwt}` : "",
                  "x-user-id": userId,
            ...
            if (statusData.status === "completed") {
              return { content: statusData.result, remainingRequests: 100 }; // Match expected format
            } else if (statusData.status === "failed") {
              throw new Error(statusData.error || "AI request failed");
            }
        ...
        if (error.name === "AbortError") {
          setError(`${promptLabel} timed out.`);
          setUserMsg(`Request was too slow and timed out.`);
          break;
        }
