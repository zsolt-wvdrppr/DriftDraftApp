...
export function useHintToast() {
  ...
  const getDomainPart = useCallback((url) => {
  ...
  const handleCheckDomain = useCallback(
    async (_url) => {
      ...
      const checkingToastId = toast.info(`Checking availability of ${url}...`, {
        ...
        classNames: { toast: "text-blue-600" },
      });
  
      try {
        const { isAvailable, suggestions, error } = await checkDomainAvailability(url);
        ...
        setDomainList((prevList) => ({
          ...prevList,
          [url]: { available: isAvailable, suggestions },
        }));
        ...
        if (isAvailable) {
          ...
          setTimeout(() => {
            if (suggestions.length > 0) {
              toast.custom(() => (
                <div className="flex flex-col p-4 bg-white shadow-lg rounded-md border border-blue-400">
  ...
  const showHintToast = useCallback(
              ...
              components={{
                a: ({ node, ...props }) => (
                  <span className="relative flex flex-wrap">
                    ...
                    {checkDomain &&
                      domainList[getDomainPart(props.href)] === undefined && (
                        <IconWorldQuestion
                    ...
                    {checkDomain &&
                      domainList[getDomainPart(props.href)] === true && (
                        <IconWorldQuestion
                    ...
                    {checkDomain &&
                      domainList[getDomainPart(props.href)] === false && (
                        <IconWorldQuestion
    ...
    [domainList, getDomainPart, handleCheckDomain]
  );

  const showWhyWeAskToast = useCallback((reason, onDismiss) => {
