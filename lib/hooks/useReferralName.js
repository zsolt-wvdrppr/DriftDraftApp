...
export const useReferralName = (userId) => {
  ...
  const updateReferralName = async (newName) => {
    ...
    if (!jwt || !newName || newName.length < 5) {
    ...
    try {
      const response = await fetch("/api/agent/update-referral-name", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${jwt}`, // ✅ Secure request
        },
      ...
      if (!response.ok)
        throw new Error(data.error || "Failed to update referral name.");
