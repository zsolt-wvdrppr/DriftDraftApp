...
if (!SECRET_KEY) {
  throw new Error(
    "Missing encryption key! Set NEXT_PUBLIC_ENCRYPTION_KEY in your .env.local file."
  );
}
...
const decryptData = (cipherText) => {
  try {
    if (!SECRET_KEY) {
      logger.error("Decryption key is missing.");
...
export function useManageSessionData(userId, steps) {
  ...
  const getSessionDataFromLocalStorage = () => {
    ...
    try {
      ...
      const sessionJSON = decryptData(encryptedSessionData);
  ...
  const initSession = async (planType = "") => {
    ...
    try {
      if (localSessionId) {
        ...
        const localData = decryptData(encryptedSessionData);
      ...
      if (!session) {
        ...
        localStorage.setItem("sessionData", encryptData(session));
  ...
  const updateSessionData = (key, value) => {
    ...
    localStorage.setItem("sessionData", encryptData(updatedSession));
  };

  const clearSessionData = () => {
    return new Promise((resolve, reject) => {
      try {
        logger.debug("Clearing session states & local storage..");
        setSessionData({
          sessionId: null,
          formData: {},
          aiGeneratedPlan: null,
  ...
  const fetchAllSessionsFromDb = async (userId) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("sessions")
        .select(
          "session_title, created_at, session_id, type, user_id, updated_at"
        )
  ...
  const initSessionFromDb = async (userId, sessionId) => {
    try {
      ...
      localStorage.setItem("sessionData", encryptData(camelCasedSession));
  ...
  // place all the plan and credit and subscription details in one state object
  // so that it can be passed to the context
  const [paidServicesData, setpaidServicesData] = useState({});

  // Fetch the user's top-up and allowance credits
  const fetchUserPaidServicesData = async (userId) => {
    if (!userId) {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          "top_up_credits, allowance_credits, promo_credits, plan_starts_at, plan_renews_at, plan_expires_at, tier"
        )
        .eq("user_id", userId)
      ...
      if (!data) {
        ...
        setTimeout(() => fetchUserPaidServicesData(userId), 2000); // Retry after 2 seconds
      ...
      // Update paidServicesData state with the fetched data
      setpaidServicesData({
        topUpCredits: data.top_up_credits,
        allowanceCredits: data.allowance_credits,
        promoCredits: data.promo_credits,
        planStartsAt: data.plan_starts_at,
        planRenewsAt: data.plan_renews_at,
        planExpiresAt: data.plan_expires_at,
        hasActiveSubscription:
          (data.plan_renews_at && data.plan_starts_at) ||
          (data.plan_starts_at && data.plan_expires_at),
        tier: data.tier,
        creditsError: null,
      });
    } catch (err) {
      ...
      setpaidServicesData({
        ...
        tier: null,
        creditsError: err.message,
      });
    }
  };

  const refreshPaidServicesData = async () => {
    ...
    await fetchUserPaidServicesData(userId);
  };
  ...
  useEffect(() => {
    if (userId) {
      ...
      fetchUserPaidServicesData(userId);
    }
  }, [userId]);
