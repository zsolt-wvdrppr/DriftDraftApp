...
//export  const supabase = (jwt) => createClient(supabaseUrl, supabaseAnonKey);
...
export function useManageSessionData(userId, steps) {
  ...
  useEffect(() => {
    // Only run once we're fully initialised, or you might prematurely create sessions
    if (!isInitialised) return;
  
    const localData = getSessionDataFromLocalStorage();

    if (!localData) {
      logger.info("No valid session or type mismatch found. Starting new session...");
      startNewSession();
    } else {
  ...
  const initSession = async () => {
    ...
    try {
      ...
      if (!session) {
        ...
        const sessionType = getPlannerTypeFromPath(pathname);
  ...
  const startNewSession = async () => {
    ...
    try {
      const newSession = await initSession(); // Ensure session initializes
