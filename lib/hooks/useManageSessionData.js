    ...
    const initSessionFromDb = async (userId, sessionId) => {
        try {
            ...
            setSessionData({
                ...
                aiGeneratedPlan: session.ai_generated_plan || null,
                sessionTitle: session.session_title || 'Untitled Session',
            });
    
            // Log the success
            logger.info(`Session initialized from DB: ${sessionId}`);
            return true; // Indicate successful initialization
        } catch (error) {
            logger.error(`Error initializing session from DB for userId: ${userId}, sessionId: ${sessionId}`, error.message);
            return false; // Indicate failure
        }
    };
