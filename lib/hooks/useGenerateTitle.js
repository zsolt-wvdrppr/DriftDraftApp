...
const useGenerateTitle = (content, updateSessionTitleInDb, userId, sessionId) => {
    ...
    useEffect(() => {
        const generateAndSaveTitle = async () => {
            if (!content || titleGenerated || !userId || !sessionId || !updateSessionTitleInDb) return; // Wait for content and avoid re-triggering

            setLoading(true);
            setError(null);

            try {
                const prompt = `Generate a very short and concise title (max 4 words), no formatting, just a simple string, based on the following content: "${content}".`;
                ...
                const _generatedTitle = data.content.trim() || "Untitled Website Plan";

                setGeneratedTitle(_generatedTitle);

                logger.debug("[TITLE GENERATOR] - Generated title:", _generatedTitle);
                
                // Call the database update function and mark title as generated
                await updateSessionTitleInDb(userId, sessionId, _generatedTitle);
    ...
    return { loading, error, generatedTitle };
};
