...
export const fetchAIHint = async ({
  ...
  const cachedResponse = sessionData?.formData?.[stepNumber]?.aiHints || null;
  const cachedPrompt = sessionData?.formData?.[stepNumber]?.prompt || null;

  if (cachedPrompt && calculateSimilarityPercentage(prompt, cachedPrompt) > 96) {
  ...
  try {
    const response = await fetch("/api/aiHintRateLimited", {
      ...
      body: JSON.stringify({ prompt, stepNumber }),
    });
    ...
    updateFormData(stepNumber, { aiHints: aiContent, prompt: prompt });
