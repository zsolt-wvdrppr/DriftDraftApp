...
export const fetchAIHint = async ({
  ...
  logger.debug("Prompt similarity:", calculateSimilarityPercentage(prompt, cachedPrompt));

  // Check if the prompt is similar to the cached prompt
  if (cachedPrompt && calculateSimilarityPercentage(prompt, cachedPrompt) > 80) {
  ...
  try {
    ...
    updateFormData(stepNumber, { aiHints: aiContent });
    updateFormData(stepNumber, { prompt })
