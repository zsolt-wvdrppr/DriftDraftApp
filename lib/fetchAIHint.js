export const fetchAIHint = async ({
  ...
  if (cachedPrompt === prompt && cachedResponse) {
    logger.info("Using cached response for identical prompt. Skipping delay.");
    setAiHints(cachedResponse);
    return; // Return early to skip the remaining logic and delay
  }

  // Apply delay only if the cached response condition is not met
  if (delay > 0) {
    logger.info(`Applying a delay of ${delay}ms before proceeding.`);
