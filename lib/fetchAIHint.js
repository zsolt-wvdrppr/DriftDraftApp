...
export const fetchAIHint = async ({
  ...
  // Step 1: Retrieve the JWT token using getSession()
  const { data: supabaseSessionData, error: sessionError } =
    await supabase.auth.getSession();

  if (sessionError || !supabaseSessionData?.session) {
    logger.warn(
      "No session or access token available. Proceeding as anonymous."
    );
  }



  const jwt = supabaseSessionData?.session?.access_token; // Securely retrieve the JWT
  ...
  if (jwt) {
    headers["Authorization"] = `Bearer ${jwt}`; // Pass the token securely
  }
  ...
  if (
    cachedPrompt &&
    calculateSimilarityPercentage(prompt, cachedPrompt) > 99.5
  ) {
  ...
  try {
    ...
    const response = await fetch("/api/aiReqRateLimited", { 
      method: "POST",
      headers,
      body: JSON.stringify({ prompt, clientData, token, sessionId: sessionData?.sessionId, pickedModel: "gemini-2.5-flash" }),
    });
    ...
    if (response.status === 429) {
      ...
      setUserMsg(
        `\n\n${message} Upgrade for higher limits and more features.`,
      );
