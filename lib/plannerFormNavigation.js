...
export const updateUrlParams = (currentStep) => {
  ...
  // Ensure the correct step is updated in query params
  currentUrl.searchParams.set('step', currentStep);

  // Preserve the hash part (OAuth tokens)
  const hash = window.location.hash;

  // Clean up possible double slashes
  const cleanPath = currentUrl.pathname.replace(/([^:]\/)\/+/g, "$1"); // Remove double slashes

  // Reconstruct the URL
  const cleanUrl = `${currentUrl.origin}${cleanPath}${currentUrl.search}${hash}`;

  // Apply the updated URL to history
