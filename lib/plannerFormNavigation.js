...
export const updateUrlParams = (currentStep) => {
  try {
      ...
      // Ensure only one slash exists between domain and path
      const cleanPath = currentUrl.pathname.replace(/\/+/g, "/");

      // Preserve the hash (OAuth token) to avoid breaking authentication
      ...
      const cleanUrl = `${currentUrl.origin}${cleanPath}${currentUrl.search}${hashFragment}`;

      // Only update history state if the new URL is different
