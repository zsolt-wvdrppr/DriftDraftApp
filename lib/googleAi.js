...
export async function fetchFromGoogleAI(_prompt) {
  ...
  const modelConfig = {
    model: useGoogleSearchGrounding ? "gemini-2.5-flash" : "gemini-2.5-flash",
    ...
    while (true) {
      ...
      if (
        ...
        for (const part of parts) {
          ...
          if (part.functionCall) {
            ...
            if (functionName === "google_search") {
              ...
              logger.warn(
                `[googleAi] - Unknown Tool Function: ${functionName}. Ignoring.`
              );
            }
          } else if (part.text) {
        ...
        if (functionCalls.length > 0) {
          ...
          logger.debug(
            "[googleAi] - Tool responses sent, waiting for model follow-up..."
          );
          continue; // Process the model's response in the next iteration
        }
      } else {
        logger.debug(
          "[googleAi] - No parts in response (likely final text response)."
        );
      ...
      if (
        ...
        logger.debug(
          "[googleAi] - Final Response Text:",
          result.response.text()
        );

        if (
          result?.response?.text() &&
          result?.response?.text() !== finalResponseText
        ) {
        ...
        logger.warn(
          "[googleAi] - No more response or result is invalid, exiting loop."
        );
        break; // Exit loop if no valid response
      }
    }

    // Only remove outer backticks if the ENTIRE response is just a code block (no other content)
    if (
...
async function fetchGoogleSearch(query) {
  try {
    const res = await fetch(
      `${process.env.URL}/api/google-search?q=${encodeURIComponent(query)}`
    );
