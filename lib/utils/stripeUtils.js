...
export async function fetchRecurringProducts() {
  const products = await stripe.products.list({
    active: true,
    expand: ["data.default_price"],
  });
  ...
  return products.data
    .filter((product) => product.default_price.recurring) // ✅ Filter only recurring products
    .map((product) => ({
      id: product.id,
      name: product.name,
      description: product.description,
      ...
      amount: product.default_price.unit_amount / 100, // Convert cents to dollars
      currency: product.default_price.currency.toUpperCase(),
      interval: product.default_price.recurring.interval, // Monthly, yearly, etc.
      creditAmount: product.metadata.credit_amount ? parseInt(product.metadata.credit_amount, 10) : 0, // Extract credit amount
    }));
}
